...
void listarComponentesAtrasados() {
  ...
  while (fscanf(arqOps,
    ...
    if ((strcmp(dataEntrada, "null") == 0) &&
        (strcmp(dataHoje, dataPreviaChegada) > 0)) {
      ...
      while (
          fscanf(
              ...
              tipo, designacao, condicao, obsComp, &garantia,
              dataAquisicao) == 11) {
        ...
        if (id == idComponente) {
          ...
          printf("\n[Componente]\nID: %d\nFornecedor: %d\nFabricante: %d\nID "
                 ...
                 id, idFornecedor, idFabricante, idPostoTrabalho, numSerie,
                 tipo, designacao, condicao, obsComp, garantia, dataAquisicao,
                 numDoc, tipoOperacao, montante, dataOperacao,
                 dataPreviaChegada);
